...
async def get_rss_feeds(retrieval_request: FeedsRetrievalRequest, response: Response):
    ...
    if not results:
        return JSONResponse(
            status_code=401,
            content={"msg": f"User {username} does not exist in the database"},
        )
    ...
    dict_results = []
    for rss in results:
        dict_rss = {"name": rss[3], "url": rss[0], "topic": rss[2], "language": rss[1]}
...
async def get_all_rss_feeds(secret_token: str, response: Response):
    ...
    fetch_user_rss_feeds_request = (
        f"""SELECT id, url, language, topic, name FROM user_sources;"""
    )
    cursor = conn.cursor()
    ...
    fetch_standard_rss_feeds_request = (
        f"""SELECT id, url, language, topic, name from standard_sources;"""
    )
    ...
    for article in articles:
        ...
        if date:  # i.e. if the date string is not empty
            ...
            if article.is_from_user:
                request = f"""INSERT INTO daily_articles(title, url, content, author, date, image, is_from_user, user_source_id) VALUES('{article.title.replace("'", '"')}', '{article.url.replace("'", '"')}', '{article.content.replace("'", '"')}', '{article.authors.replace("'", '"')}', '{date}', '{article.image}', true, {article.source_id})"""
            else:
                request = f"""INSERT INTO daily_articles(title, url, content, author, date, image, is_from_user, standard_source_id) VALUES('{article.title.replace("'", '"')}', '{article.url.replace("'", '"')}', '{article.content.replace("'", '"')}', '{article.authors.replace("'", '"')}', '{date}', '{article.image}', false, {article.source_id})"""
        else:
            if article.is_from_user:
                request = f"""INSERT INTO daily_articles(title, url, content, author, image, is_from_user, user_source_id) VALUES('{article.title.replace("'", '"')}', '{article.url.replace("'", '"')}', '{article.content.replace("'", '"')}', '{article.authors.replace("'", '"')}', '{article.image}', true, {article.source_id})"""
            else:
                request = f"""INSERT INTO daily_articles(title, url, content, author, image, is_from_user, standard_source_id) VALUES('{article.title.replace("'", '"')}', '{article.url.replace("'", '"')}', '{article.content.replace("'", '"')}', '{article.authors.replace("'", '"')}', '{article.image}', false, {article.source_id})"""
